...
export const BlogCard = ({
    ...
    return <Link to={`/blog/${id}`}>
        <div className="p-4 border-b pb-4 w-screen max-w-screen-md cursor-pointer">
            ...
            <div className="flex justify-center flex-col text-xs pl-2">
                <Circle></Circle>
            </div>
...
export function Avatar({ name, size = "small" }: { name:string, size: "small" | "big"}){
    return  <div className={`${size === "small" ? "w-6 h-6" : "w-10 h-10"} relative inline-flex items-center justify-center overflow-hidden bg-gray-100 rounded-full dark:bg-gray-600`}>
